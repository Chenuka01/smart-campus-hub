...
export default function FavoritesPage() {
  ...
  const favoriteFacilities = allFacilities.filter(f => isFavorite(f.id));

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        <div className="w-10 h-10 rounded-full border-2 border-violet-500/30 border-t-violet-500 animate-spin" />
        <p className="text-sm text-slate-500 animate-pulse">Loading your favorites...</p>
      </div>
    );
  }
      ...
      <motion.div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">Resources you have saved for quick access</p>
          </div>
        </div>
      </motion.div>
      ...
      <AnimatePresence>
        {favoriteFacilities.length > 0 ? (
          ...
          <motion.div 
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-center py-24 px-4 border border-white/5 rounded-2xl bg-white/5 backdrop-blur-sm"
          >
            ...
            <p className="text-sm text-slate-400 mb-6 max-w-md mx-auto">
              You haven't saved any resources to your favorites. Click the heart icon on any resource to save it here for quick access later.
            </p>
            <Link to="/facilities">
              <NeuButton variant="secondary" size="md">
                Browse Resources
              </NeuButton>
            </Link>
          </motion.div>
