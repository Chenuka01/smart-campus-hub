...
import {
  Building2, ArrowLeft, Heart, MapPin, Users,
  Monitor, Camera, Laptop, FlaskConical, Presentation
} from 'lucide-react';
...
export default function FavoritesPage() {
  ...
  // Filter only those that are favorited
  const favoriteFacilities = facilities.filter(f => isFavorite(f.id));
  ...
  return (
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="space-y-6 pb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-extrabold text-white tracking-tight capitalize">
              My <span className="text-gradient hover:text-rose-500 transition-colors">Favorites</span>
            </h1>
            <p className="text-slate-400 text-sm mt-1">Your saved facilities and resources</p>
          </div>
        </div>
      </motion.div>
      ...
      {favoriteFacilities.length === 0 ? (
         <motion.div variants={itemVariants} className="flex flex-col items-center justify-center h-64 gap-3">
           <Heart className="w-12 h-12 text-slate-600 mb-2" />
           <p className="text-slate-400 text-center">You haven't added any favorites yet.</p>
           <Link to="/facilities">
             <button className="mt-2 text-rose-500 hover:text-rose-400 font-medium">Browse Facilities</button>
           </Link>
         </motion.div>
      ) : (
